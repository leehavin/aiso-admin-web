...
defineOptions({ name: "system_locale" });
...
//表单操作对象
const refCreateInfo = ref<InstanceType<typeof CreateInfo>>();
//编辑操作对象
const refEditInfo = ref<InstanceType<typeof EditInfo>>();
//检索表单
...
async function findList() {
  try {
    state.loading = true;
    const result = await SysLocaleService.findList(state.page, state.size, state.search.vm, state.search.sort);
    state.loading = false;
...
async function deleteList(id?: string) {
  ...
  try {
    state.loading = true;
    const result = await SysLocaleService.deleteList(ids);
    state.loading = false;
    if (result.code != 200) return;
...
function exportExcel() {
  SysLocaleService.exportExcel(state.search.vm, state.search.sort);
}
</script>
  ...
  <PageContainer>
    <TableCurd
      ...
      <template #toolbar-left>
        ...
        <a-button type="primary" @click="() => refCreateInfo?.open()" v-if="power.insert">
          <template #icon>
      ...
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'action'">
          <a href="javascript:;" @click="() => refEditInfo?.open(record.id)" v-if="power.update">编辑</a>
    ...
    <!-- Info -->
    <CreateInfo ref="refCreateInfo" :onSuccess="() => findList()" />
    <EditInfo ref="refEditInfo" :onSuccess="() => findList()" />
  </PageContainer>
</template>
