...
defineOptions({name: "system_post"});
...
async function findList() {
  try {
    state.loading = true;
    const result = await SysPostService.findList(state.page, state.size, state.search.vm, state.search.sort);
    state.loading = false;
...
async function deleteList(id?: string) {
  ...
  try {
    state.loading = true;
    const result = await SysPostService.deleteList(ids);
    state.loading = false;
    if (result.code != 200) return;
...
function exportExcel() {
  SysPostService.exportExcel(state.search.vm, state.search.sort);
}
</script>
