...
class Tools {
  ...
  static notice = {
     ...
     * @returns
     */
    success: (text: string, title: string = "提示") => AntdvMotification.success({ message: title, description: text }),
     ...
     * @returns
     */
    warning: (text: string, title: string = "提示") => AntdvMotification.warning({ message: title, description: text }),
     ...
     * @returns
     */
    error: (text: string, title: string = "提示") => AntdvMotification.error({ message: title, description: text }),
     ...
     * @returns
     */
    info: (text: string, title: string = "提示") => AntdvMotification.info({ message: title, description: text }),
  };
   ...
   * @param tokenValue
   */
  static setAuthorization(tokenValue: string, refreshTokenValue: string): void {
    const cookies = useCookies([AppConsts.tokenKey, AppConsts.refreshTokenKey]);
    console.log(cookies,cookies.getAll(),location.origin)
    ...
    cookies.set(AppConsts.refreshTokenKey, refreshTokenValue, {
      domain: location.origin,
      path: "/",
      //   secure: true,
      sameSite: "none",
    ...
    localStorage.setItem(AppConsts.tokenKey, tokenValue);
    localStorage.setItem(AppConsts.refreshTokenKey, refreshTokenValue);
  ...
  static getAuthorization(): string {
    ...
    let token = cookies.get<string>(AppConsts.tokenKey);

    if (!token) {
      token = localStorage.getItem(AppConsts.tokenKey)!;
    }
  ...
  static getRefreshToken() {
    const cookies = useCookies([AppConsts.tokenKey, AppConsts.refreshTokenKey]);
    let refreshToken = cookies.get<string>(AppConsts.refreshTokenKey);

    if (!refreshToken) {
      refreshToken = localStorage.getItem(AppConsts.refreshTokenKey)!;
    }
  ...
  static removeAuthorization(): boolean {
    ...
    cookies.remove(AppConsts.refreshTokenKey);
    localStorage.removeItem(AppConsts.refreshTokenKey);
    return true;
  }
